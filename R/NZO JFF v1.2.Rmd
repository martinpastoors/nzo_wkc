...
paths <- list(
  ...
  scenariodir = "C:/Users/MartinPastoors/Martin Pastoors/NZO - General/WKC/adviezen/2025-xx Fact Finding beschermde gebieden/scenarios/v1.2"
)
...
# Bijgewerkte JFF rapportage voor het NZO

10 juni 2025

Martin Pastoors

In de JFF presentatie van 28 mei 2025 zijn de resultaten van de JFF gepresenteerd aan de NZO werkgroep beschermde gebieden (JFF v1.1). Na afloop van de presentatie zijn door de visserij-sector vragen gesteld over het gebied dat was meegenomen voor het "visserijvoorstel Friese Front". In de JFF zijn we uitgegaan van de GIS bestanden zoals aangemaakt tijdens de NZO besprekingen van eind 2024. Daarin werd gesproken over een voorstel waarbij 0.4% zou worden gerealiseerd op respectievelijk het Friese Front, de Bruine Bank en de Voordelta. De 0.4% op het Friese Front kwam overeen met een gebied van ongeveer 220 km2 en dat is het gebied dat werd weergegeven in de door de JFF gebruikte GIS bestanden. 

Het oorspronkelijke voorstel vanuit de visserijsector was echter om een groter gebied te sluiten van het Friese Front. In het format "Visserijvoorstel Friese Front" was door WMR berekend dat het zou gaan om een gebied van ongeveer 489 km2. De precieze coordinaten van het gebied waren echter niet aangegeven in het format. Om toch de resultaten te kunnen evalueren van het oorspronkelijke voorstel vanuit de visserij (minus de zuidelijke strook waar olie/gaswinning plaatsvindt), worden hier onder de belangrijkste resultaten van de JFF opnieuw berekend rekening houdend met het visserijvoorstel van het Friese Front (JFF v1.2). 
...
Links de gebieden zoals gebruikt in JFF v1.1 rapportage, rechts v1.2. 
...
p1 <-
  ...
  geom_sf(data= jff_comb_v1.1 %>% filter(SITE_NAME %notin% c("EEZ", "Doggerbank 619")), 
          fill="gray20", linewidth=0.2, alpha=0.4) +
  geom_sf(data= jff_comb_v1.1 %>% filter(SITE_REG != "open" & SITE_NAME != "Doggerbank 619"), 
          aes(fill=SITE_REG), linewidth=0.2, alpha=0.4) +
  geom_sf(data= jff_comb_v1.1 %>% filter(SITE_NAME %in% c("Doggerbank 619")), 
...
p1 + p2 + 
  plot_layout(nrow = , guides="collect") &
...
Oppervlaktes van de verschillende beschouwde gebieden op het Friese Front. 
...
jff_comb %>% 
  ...
  ggplot() +
    ...
    # geom_sf(data=wsergrid, fill=NA, colour="gray", linewidth=0.5) +
    # geom_sf_text(data=wserextra, aes(label=round(Value, digits=2)), size=4) +
    # geom_sf_text(data=wsergrid, aes(label=as.integer(gridID)), size=4) +
    # geom_sf_text(data=jff_wsergrid, aes(label=round(area_km2, digits=2)), size=4) +
    # coord_sf(xlim=xlim, ylim=ylim)  
    coord_sf(xlim=c(4, 5.7), ylim=c(53.4, 54.3))  
...
Afkomstig uit de Partiele Herziening Noordzee (PH), NL Olie en Gas (ref) en N2000/KRM. 

```{r, fig.asp=0.9, echo=FALSE, message=FALSE, warning=FALSE, comment=NA}
...
*Habitat types in de Noordzee*

De Habitattypes (KRM ecotoop) in de Noordzee zijn gebaseerd op EUSeaMap en onderverdeeld in 7 klassen: Diep grof sediment, Diep slibrijk, Diep zand, Ondiep grof sediment, Ondiep slibrijk, Ondiep zand en Overig. Links de habitats zoals direct afkomstig van EUSeaMap en rechts het dominante habitat (grootste oppervlak) per WSER gridcel.   
...
p2 <- 
  ...
  ggplot() +
  theme_publication() +
  geom_sf(aes(fill=krm_ecotoop), linewidth=0.1, alpha=0.5) +
  geom_sf(data=jff_comb, fill=NA, linewidth=0.1, colour="black") +
  guides(fill = guide_legend(nrow = 2)) +
  coord_sf(xlim=c(bb[1],bb[3]), ylim=c(bb[2],bb[4])) 
...
Van de JFF v1.1 scenario's, hebben we hier alleen de volgende scenario's herhaalt in JFF v1.2, ter vergelijking: 
...
*scenario oppervlaktes en percentages*

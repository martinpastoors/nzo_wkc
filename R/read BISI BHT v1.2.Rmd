...
wsergebieden <- 
  readxl::read_excel(file.path(wserdir, "WSER gebieden gecombineerd v1.2.xlsx"), col_names = TRUE) 
...
jff_wsergrid <-
  sf::st_intersection(jff_comb, wsergrid) %>% 
...
#   geom_sf(data=jff_comb, fill=NA, linewidth=0.1, colour="black") +
#   coord_sf(ylim=c(53,55), xlim=c(4,6))
...
writexl::write_xlsx(listed_df, path=file.path(wserdir, "WSER gebieden met BISI scores v1.2.xlsx"))
...
jff_comb %>% 
  ...
  ggplot() +
    ...
    # coord_sf(xlim=xlim, ylim=ylim)  
    coord_sf(xlim=c(5, 5.7), ylim=c(53.6, 53.9))  
